...
a,a1,a2,... ::= n | x | a1 + a2 | a1 - a2 | a1 * a2
b,b1,b2,... ::= true | false | b1 && b2 | ~b | a1 == a2 | a1 <= a2
c,c1,c2,... ::= SKIP | IF b THEN p1 ELSE p2 | WHILE b DO p END | x ::= a
p,p1,p2,... ::= E | c ;; p
^kolcsonosen induktiv tipuskent lehet csak megadni (mutual inductive type)

X ::= 1 ;; Y ::= 2 ;; Z ::= 3

X ::= 1 ;; (Y ::= 2 ;; (Z ::= 3 ;; E))

(X ::= 1 ;; Y ::= 2 ;; E) ;; Z ::= 3 ;; E <- nincs ertelme
*)
...
intro. unfold ex7 in H. inversion H. Qed.
Definition ex8 : aexp := 1 + 3 * 2 + avar X.
Definition ex9 := 1 + alit ((3 * 2)%nat) + X.
Example ex8VsEx9 : ex8 <> ex9.
intro. unfold ex8 in H.  unfold ex9 in H. simpl in H. discriminate H. Qed.

(* Peldaprogramok: *)

Definition prog1 : cmd := X ::= 1 ;; X ::= 2.
(* Ird at prog1'-t ugy, hogy prog1-el megegyezzen, de a csunya jelolessel! *)
Definition prog1' : cmd := cseq (cassign X 1) (cassign X (alit 2)).
(*
     cseq
      /\
     /  \
...
Definition proga : cmd := X ::= 1 ;; (Y ::= 2 ;; Z ::= 1).
Definition proga' : cmd := (X ::= 1 ;; Y ::= 2) ;; Z ::= 1.
Example progaTest : proga <> proga'. intro. discriminate. Qed.

Definition progb : cmd := X ::= 1 ;; X ::= 2.
Definition progb' : cmd := X ::= 2.
Example progbTest : progb <> progb'. intro. discriminate. Qed.
...
(* Ird at fac'-t ugy, hogy fac-al megegyezzen, de a csunya jelolessel! HF *)
Definition fac' : cmd :=
  X ::= 1 ;;
  Y ::= 1 ;;
  WHILE Y <= Z DO
...
Example bevalTest3 : beval (X <= (1 + 2 - 3)) empty = true. reflexivity. Qed.

(* Mi lenne, ha bool tipusu valtozoink is lennenek?
 - C stilus: state = string -> nat, evalb (bvar x) s = if s x ?= 0 then false else true
 - state = (string -> Either nat bool).
 - state = (string -> nat) x (string -> bool)
     X ::= 3 ;; X ::= true
 - ...
*)
...
(* let (q, s') = beval b s in if q then ceval c1 s' else ceval c2 s' *)
(*  if (++x > 3) { ... } *)
(*  if (x++ > 3) { ... } *)
...
Compute ceval fac (update Z 5 empty) 10 X.
Compute ceval fac (update Z 100 empty) 10 X.
