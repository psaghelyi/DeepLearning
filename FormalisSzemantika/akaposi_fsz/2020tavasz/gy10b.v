...
Compute fromResult (ceval (WHILE true DO X ::= X + 1 END) empty 15) X.

Compute fromResult (ceval (WHILE X <= 3 DO X ::= X + 1 END) empty 10) X.
...
(* Letezik vegtelen ciklus, ami konstans ugyanazt csinalja. *)
(*
...
- simpl. destruct n.
-- simpl. reflexivity.
-- simpl. exact IHn. (* igy is mukodik: remember (S n). rewrite -> Heqn0 at 1. simpl. exact IHn. *)
Qed.
...
(*
  informalis/Haskell/ML denotacios szemantika: fun c s => match c with
   | SKIP                      => s
   | x ::= a                   => update x (aeval a s) s   -- (x::=a , s , update x (aeval a s) s)
   | c1 ;; c2                  => s'' where { s' = ceval c1 s ; s'' = ceval c2 s' }
   | TEST b THEN c1 ELSE c2 FI => if beval b s then s' else (ceval c2 s) where s' = ceval c1 s
...
Lemma noNAT : ~ NAT.
intro. induction H. exact IHNAT.
Qed.

Example ex1 (s : state) : | X ::= 1 + 41 , s |=> update X 42 s.
apply cevalb_assign. Qed.

(* Coq-ban nem bizonyithato, hogy

  (update X 4 (update X 3 empty)) = update X 3 empty  (Kovetkezik a fuggveny extenzionalitas axiomabol.)

  De coq-ban az sem bizonyithato, hogy
...
(* destruct-olni kell az X <= Y denotacios szemantikajan, kell eqn:H is *)
