...
levezetesi fa (tudom, hogy s X = 1):
  ...
  plus (var X) (var Y) , s => plus (lit 1) (var Y) => plus (lit 1) (lit 2) => lit 3

  plus (var X) (var Y) , s => plus (var X) (lit 2) => plus (lit 1) (lit 2) => lit 3
*)

(* Int(true), String(false)
Inductive exp : bool -> Type :=
 | lit : nat -> exp true
 | var : string -> exp ?
 | plus : exp true -> exp true -> exp true
 | ures : exp false
 | strlit : string -> exp false.
GADT: generalised algebraic datatype = indexed inductive type
*)
...
unfold e1. Check eval_plus_lhs.
apply eval_plus_lhs. Check eval_var.
apply eval_var.
Qed.

Example e1evalStep2 : plus (lit 4) (lit 3) , (update X 4 empty) => lit 7.
Check eval_plus_fin. apply eval_plus_fin.
Qed.
